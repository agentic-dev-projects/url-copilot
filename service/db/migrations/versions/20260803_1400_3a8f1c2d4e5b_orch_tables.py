...
Creates the 6 orchestrator tables (orch_ prefix) that co-locate with the
URL shortener service tables in the same PostgreSQL instance.

Table creation order respects foreign key dependencies:
  1. orch_runs          — standalone, one row per orchestration run
  2. orch_stage_results — FK → orch_runs
  3. orch_audit_events  — FK → orch_runs (append-only — never UPDATE/DELETE)
  4. orch_metrics       — FK → orch_runs
  5. orch_memory        — FK → orch_runs (nullable — seeds have no run)
  6. orch_cache         — standalone response cache

Why no orch_users table?
------------------------
User identity is resolved at auth time by TokenAuthenticator, which reads
orchestrator/config/users.yaml directly.  Every other component that needs
to know "who is this person" receives a CurrentUser dataclass — it never
queries a DB table.  Maintaining a duplicate orch_users table alongside
users.yaml creates a dual source of truth problem with no compensating
benefit in this prototype.  The actor field in orch_audit_events stores
github_login as a plain string, which is sufficient for the audit trail.
In production, replace the YAML lookup with an OAuth/SSO API call inside
TokenAuthenticator — no other code changes required.
...
def upgrade() -> None:
    # ------------------------------------------------------------------ #
    # 1. orch_runs — One row per orchestration run                        #
    ...
    op.create_table(
        ...
        sa.Column('triggered_by', sa.String(100), nullable=False),   # github_login from users.yaml
    ...
    # ------------------------------------------------------------------ #
    # 2. orch_stage_results — One row per stage execution attempt         #
    ...
    # ------------------------------------------------------------------ #
    # 3. orch_audit_events — Append-only SDLC event log                  #
    ...
    # ------------------------------------------------------------------ #
    # 4. orch_metrics — Per LLM/tool call observability                  #
    ...
    # ------------------------------------------------------------------ #
    # 5. orch_memory — Cross-run persistent memory                       #
    ...
    # ------------------------------------------------------------------ #
    # 6. orch_cache — LLM response cache                                 #
